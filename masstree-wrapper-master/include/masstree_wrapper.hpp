...
// Above is the masstree_wrapper source code, below is adjusted edition for debugging. 
...
public:
  ...
  MasstreeWrapper(int thread_id) { this->thread_init(thread_id); }
  ...
  bool get_value(const char *key, std::size_t len_key, T *ret_value) {
    unlocked_cursor_type lp(table_, key, len_key);
    bool found = lp.find_unlocked(*ti);
    if (found){
      *ret_value = lp.value();
  ...
  bool get_value_and_get_nodeinfo_on_failure(const char *key, std::size_t len_key,
                                           node_info_t &node_info, T *ret_value) {
    unlocked_cursor_type lp(table_, key, len_key);
    bool found = lp.find_unlocked(*ti);
    if (found){
      *ret_value = lp.value();
