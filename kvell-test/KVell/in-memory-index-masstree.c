...
masstree_t *items_locations[20];
static __thread index_entry_t tmp_entry;
static pthread_spinlock_t *items_location_locks;

index_entry_t *masstree_worker_lookup(int worker_id, void *item) {
   uint64_t hash = get_prefix_for_item(item);
   int res = masstree_find(items_locations[worker_id], (unsigned char*)&hash, sizeof(hash), &tmp_entry);
   if(res)
      return &tmp_entry;
   else
      return NULL;
}
void masstree_worker_insert(int worker_id, void *item, index_entry_t *e) {
   uint64_t hash = get_prefix_for_item(item);

   pthread_spin_lock(&items_location_locks[worker_id]);
   //print_item(worker_id,item);
   masstree_insert(items_locations[worker_id], (unsigned char*)&hash, sizeof(hash), e);
...
void masstree_worker_delete(int worker_id, void *item) {
   ...
   pthread_spin_lock(&items_location_locks[worker_id]);
   masstree_delete(items_locations[worker_id], (unsigned char *)&(hash), sizeof(hash));
   pthread_spin_unlock(&items_location_locks[worker_id]);
...
void masstree_index_add(struct slab_callback *cb, void *item) {
   ...
   masstree_worker_insert(get_worker(new_entry->slab), item, new_entry);
   // masstree_worker_insert(get_worker(cb->slab), item, new_entry);
}
...
struct index_scan masstree_init_scan(void *item, size_t scan_size) {
   ...
   for(size_t w = 0; w < nb_workers; w++) {
      ...
      res[w] = masstree_find_n(items_locations[w], (unsigned char *)&(hash), sizeof(hash), scan_size);
...
/*struct index_scan masstree_init_scan(void *item, size_t scan_size) {
   ...
   masstree_t *b = masstree_create();
   for(size_t w = 0; w < nb_workers; w++) {
      for(size_t i = 0; i < res[w].nb_entries; i++) {
         masstree_insert(b, (unsigned char*)&(res[w].hashes[i]), sizeof(res[w].hashes[i]), &res[w].entries[i]);
      }
   }
   scan_res = masstree_find_n(b, (unsigned char *)&(hash), sizeof(hash), scan_size);
   for(size_t w = 0; w < nb_workers; w++) {
      free(res[w].hashes);
      free(res[w].entries);
   }
...
void masstree_init(void) {
   ...
   for(size_t w = 0; w < get_nb_workers() ; w++) {
      //items_locations[w] = masstree_create(w);
